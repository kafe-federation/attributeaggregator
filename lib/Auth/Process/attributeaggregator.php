...
class sspmod_attributeaggregator_Auth_Process_attributeaggregator extends SimpleSAML_Auth_ProcessingFilter
{
    ...
    public function process(&$state)
    {
        ...
        try {
            ...
            //$this->mergeAttributes($state, Array( 'test'=> ['hello'] ));
            ...
            //$query->setIssuer('https://testssp.kreonet.net/sp/simplesamlphp');
            ...
            $attr = $this->getRequestedAttributes();
            //$attr['email'] = $state['Attributes']['urn:oid:0.9.2342.19200300.100.1.3'];
            //$attr['urn:oid:1.3.6.1.4.1.5923.1.5.1.1'] = Array('test:group');
            //SimpleSAML\Logger::info('[attrs]'.json_encode($attr));

            $query->setNameId($nameid);
            $query->setAttributeNameFormat($this->attributeNameFormat);
            $query->setAttributes($attr); // may be empty, then it's a noop
        ...
        //    SimpleSAML\Logger::info(strip_tags($query->toUnsignedXML()));
            ...
            foreach($assertions as $assertion) {
                ...
                SimpleSAML\Logger::info("[aa] ".json_encode($assertion->getAttributes()));
                $this->mergeAttributes($state, $assertion->getAttributes());
            }
        } catch (Exception $e) {
            SimpleSAML\Logger::error("Attribute query failed: ".$e->getMessage());
            if ($this->required) {
                throw $e;
            }
        }
    ...
    private function getRequestedAttributes() {
        if (!sizeof(array_keys($this->attributes))) {
            return array();
        }
